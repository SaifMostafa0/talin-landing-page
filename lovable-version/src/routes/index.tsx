...
import fullLogoAsset from "../assets/talin-full-dark.png";
...
function Index() {
  ...
  return (
    <main className="overflow-clip bg-background text-foreground">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-nav-border bg-nav/95 backdrop-blur-md">
        <div className="mx-auto flex h-20 max-w-screen-2xl items-center justify-between px-5 md:px-10 lg:px-16">
          <a href="#top" aria-label="Talin home" className="focus-ring inline-flex">
            <img
          ...
          <nav aria-label="Primary navigation" className="hidden items-center gap-7 lg:flex">
            {navItems.map(([label, id]) => (
              <a key={id} href={`#${id}`} className="nav-link focus-ring">
                {label}
              </a>
            ))}
          </nav>
        ...
        {menuOpen && (
          <nav aria-label="Mobile navigation" className="border-t border-nav-border bg-nav px-5 py-6 lg:hidden">
            <div className="flex flex-col gap-1">
              {navItems.map(([label, id]) => (
                <a
                  ...
                  className="focus-ring border-b border-nav-border py-4 font-body text-base text-nav-foreground"
                >
                  {label}
                </a>
              ))}
      ...
      <section id="top" className="hero-field relative flex min-h-[88svh] items-end pt-28">
        ...
        <div className="relative z-10 mx-auto grid w-full max-w-screen-2xl gap-12 px-5 pb-16 md:px-10 md:pb-24 lg:grid-cols-[1fr_0.72fr] lg:px-16">
          ...
          <div className="flex max-w-xl flex-col justify-end lg:pb-3">
            <p className="text-xl leading-relaxed text-hero-body md:text-2xl">
      ...
      <section id="about" className="section-pad bg-surface-light scroll-mt-20">
        <div className="content-grid">
          ...
          <div>
            <div className="grid gap-px bg-strong-border md:grid-cols-2">
              <article className="bg-surface-light p-7 md:p-10">
            ...
            <div className="mt-14">
              ...
              <div className="mt-7 grid gap-px bg-strong-border md:grid-cols-3">
                <article className="bg-surface-light p-7 md:p-8">
                  ...
                  <ul className="mt-5 flex flex-wrap gap-2">
                    {["Human-centricity", "Foresight", "Intelligence", "Partnership", "Responsibility", "Business value"].map((value) => (
                      <li key={value} className="value-chip">{value}</li>
                    ))}
                  </ul>
                </article>
                <article className="bg-surface-light p-7 md:p-8">
                  ...
                  <ul className="mt-5 flex flex-wrap gap-2">
                    {["Measurable impact", "Business value", "Sustainable growth"].map((commitment) => (
                      <li key={commitment} className="value-chip">{commitment}</li>
                    ))}
                  </ul>
                </article>
                <article className="bg-surface-light p-7 md:p-8">
      ...
      <section id="services" className="section-pad scroll-mt-20">
        <div className="mx-auto max-w-screen-2xl px-5 md:px-10 lg:px-16">
          <div className="content-grid">
            ...
            <p className="max-w-3xl text-xl leading-relaxed text-muted-foreground md:text-2xl">We work in two connected modes. Advisory helps organizations define direction, strategy, and readiness. Delivery builds, implements, and embeds the solutions that advisory defines.</p>
          </div>
          <div className="mt-16 grid border-l border-t border-border md:grid-cols-2 xl:grid-cols-3">
            {services.map(({ icon: Icon, number, title, copy }) => (
              <article key={title} className="service-card group border-b border-r border-border p-7 md:p-9">
                ...
                <h3 className="mt-20 max-w-xs font-display text-2xl leading-tight font-semibold text-primary md:text-3xl">{title}</h3>
      ...
      <section id="approach" className="section-pad bg-navy text-navy-foreground scroll-mt-20">
        <div className="mx-auto max-w-screen-2xl px-5 md:px-10 lg:px-16">
          <div className="content-grid">
            <div>
          ...
          <div className="mt-20">
            ...
            <div className="mt-7 grid border-l border-t border-navy-border md:grid-cols-5">
              {steps.map(([number, title, copy]) => (
                <article key={title} className="group relative min-h-64 border-b border-r border-navy-border p-6 md:p-7">
                  <span className="font-body text-xs font-semibold text-navy-accent">{number}</span>
                  <div className="absolute left-0 top-0 h-1 w-0 bg-navy-accent transition-all duration-500 group-hover:w-full" />
                  <h3 className="mt-20 font-display text-2xl font-semibold">{title}.</h3>
                  <p className="mt-3 text-sm leading-relaxed text-navy-muted">{copy}</p>
                </article>
              ))}
            </div>
          </div>
      ...
      <section id="leadership" className="section-pad bg-surface-light scroll-mt-20">
        <div className="mx-auto max-w-screen-2xl px-5 md:px-10 lg:px-16">
          <div className="content-grid">
            <div>
              <p className="section-index">04 / Leadership</p>
              <h2 className="section-title mt-5">The Minds Behind Talin</h2>
            </div>
      ...
      <section id="contact" className="contact-field section-pad relative scroll-mt-20">
        <div aria-hidden="true" className="talin-star contact-star"><span /></div>
        <div className="relative z-10 mx-auto max-w-screen-2xl px-5 md:px-10 lg:px-16">
          <p className="section-index text-contact-accent">05 / Contact</p>
          <div className="mt-8 grid gap-12 lg:grid-cols-[1fr_0.7fr] lg:items-end">
            <div>
            ...
            <div className="border-l border-contact-border pl-7 md:pl-10">
              <p className="eyebrow text-contact-accent">Contact details coming soon</p>
              <p className="mt-5 leading-relaxed text-contact-muted">Our direct contact details, Cairo office address, and LinkedIn pages are being confirmed before launch.</p>
              <span className="button-disabled mt-8 inline-flex" aria-disabled="true">Get in Touch</span>
            </div>
          </div>
        </div>
      </section>

      <footer className="border-t border-strong-border bg-background py-10">
        <div className="mx-auto flex max-w-screen-2xl flex-col gap-8 px-5 md:flex-row md:items-end md:justify-between md:px-10 lg:px-16">
          <img src={fullLogoAsset} alt="Talin by CID Consulting" className="h-auto w-52 object-contain object-left" />
          <p className="font-body text-xs text-muted-foreground">© 2026 Talin. All rights reserved.</p>
        </div>
      </footer>
    </main>
  );
...
function Leader({ name, role, initials, children }: { name: string; role: string; initials: string; children: React.ReactNode }) {
  return (
    <article className="grid gap-7 py-10 md:grid-cols-[9rem_1fr] md:py-12">
      <div className="relative flex aspect-square w-32 items-center justify-center overflow-hidden bg-primary text-primary-foreground">
        ...
        <span className="absolute bottom-2 left-2 right-2 bg-primary px-2 py-1 text-center font-body text-[0.6rem] font-semibold uppercase tracking-widest text-primary-foreground">Photo pending</span>
      </div>
      <div>
        ...
        <p className="mt-5 max-w-3xl leading-relaxed text-muted-foreground">{children}</p>
      </div>
    </article>
  );
}
